...
public class KnuckleItem extends SpellWeaponItem {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if(player.hasStatusEffect(Effects.STONE_HAND.registryEntry)){
            if (randomrange_stun >= stonhand_stun_chance ){
                target.addStatusEffect(new StatusEffectInstance(MRPGCEffects.STUNNED.registryEntry, stun_duration));
            }
        }
        stack.damage(1, attacker, EquipmentSlot.MAINHAND);
        return true;
